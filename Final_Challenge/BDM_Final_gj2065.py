...
def distances(x, dict_cbg_latlong):
    ...
    sum_dis = sum(map(lambda cbg: haversine2(dict_cbg_latlong.get(cbg), poi_cbg_latlong), x[1]))
...
def format_month_distance(x):
    ...
    return Row(cbg_fips, date_distances.get('2019-03', ''), date_distances.get('2019-10', ''),date_distances.get('2020-03', ''), date_distances.get('2020-10', ''))
...
if __name__ == '__main__':
    ...
    spark = SparkSession.builder.appName("BDM_Final_gj2065").getOrCreate()
    ...
    nyc_supermarkets_data = spark.read \
        .option('header', True) \
        .option('delimiter', ',') \
    ...
    weekly_patterns_data = weekly_patterns_data.join(nyc_supermarkets_data,['placekey']).filter('place_id is not null')
    ...
    weekly_patterns_data = weekly_patterns_data.rdd.map(lambda row: [row['poi_cbg'], row['visitor_home_cbgs'],parse_year_month(row['date_range_start_flag'],row['date_range_end_flag'])]) \
    ...
    result = spark.createDataFrame(weekly_patterns_data\
        .map(lambda x: distances(x, map_cbg_lat_lon)) \
        .reduceByKey(lambda x, y: [ x[0] + y[0], x[1] + y[1] ]) \
        .map(lambda x: (x[0][0], [x[0][1], round(x[1][0] / x[1][1] * 0.621371, 2)]))\
        .groupByKey() \
        .map(lambda group: format_month_distance(group)),['cbg_fips','2019-03','2019-10','2020-03','2020-10']) \
        .orderBy('cbg_fips')\
